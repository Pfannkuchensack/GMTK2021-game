  ...
  data: () => {
    return {
      ...
      itemstypes: [
        ...
        { type: 3, name: "Geschmolzendes Eisen", ticks: 20, nexttype: 4 },
        { type: 4, name: "Zerschmolzendes Eisen", ticks: 1, nexttype: 0 },
  ...
  methods: {
    ...
    ticksystem() {
      ...
      this.interactionLocations.forEach((location) => {
        if (typeof location.inventory != "undefined" && location.inventory.length > 0) {
          location.inventory.forEach((item, i, a) => {
            item.ticks--;
			if(item.ticks == 0)
			{
				if(this.itemstypes.find((e) => e.type == item.type).nexttype == 0)
					a.splice(i, 1);
				item.type = this.itemstypes.find((e) => e.type == item.type).nexttype;
				item.ticks = this.itemstypes.find((e) => e.type == item.type).ticks
			}
          });
        }
      });
